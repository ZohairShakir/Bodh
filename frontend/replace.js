const fs = require('fs');
let lines = fs.readFileSync('app/dashboard/page.tsx', 'utf8').split('\n');
...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('{/* HQ View: Personlized Student Dashboard */}')) {
        startIndex = i;
    }
    if (lines[i].includes('{/* History View */}')) {
        endIndex = i;
        break;
    }
}
...
if (startIndex !== -1 && endIndex !== -1) {
    const numToDelete = endIndex - startIndex;
    lines.splice(startIndex, numToDelete, replacement);
    fs.writeFileSync('app/dashboard/page.tsx', lines.join('\\n'));
    console.log("Successfully replaced " + numToDelete + " lines.");
} else {
