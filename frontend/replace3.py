...
try:
    ...
    start_str = "HQ View: Personlized Student Dashboard"
    end_str = "History View"

    start_idx = text.find(start_str)
    end_idx = text.find(end_str)

    if start_idx != -1 and end_idx != -1:
        # Step back to catch the exact start of the line and end
        real_start = text.rfind("                    {/*", 0, start_idx)
        real_end = text.rfind("                    {/*", 0, end_idx) # finds the history start
        
        new_text = text[:real_start] + replacement + text[real_end:]
