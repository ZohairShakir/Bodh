const fs = require('fs');
const lines = fs.readFileSync('app/dashboard/page.tsx', 'utf8').split('\n');

const replacement = `                    {/* Arena View */}
                    {view === 'arena' && (
...
let actualReplacementContent = "";
for (let line of replacement.split('\n')) {
    actualReplacementContent += (line.endsWith('\r') ? line : line + '\r') + '\n';
}

if(lines[655].includes("HQ View:") && lines[797].includes("History View")) {
    lines.splice(655, 142, actualReplacementContent.trimEnd());
    fs.writeFileSync('app/dashboard/page.tsx', lines.join('\n'));
    console.log("Successfully hard-spliced HQ View directly into Arena View!");
} else {
    console.log("WARNING! Index mismatch! Line 655 is:", lines[655]);
    console.log("Line 797 is:", lines[797]);
}
